...
sshd's sandbox profile and TCC grants are settable only through the GUI. The playbook detects when these are missing and halts on an interactive `pause:` with a 10-minute timeout and recovery instructions, rather than half-working silently.

- **autofs SMB map** — autofs only reads `/etc/auto_smb` when `/etc/auto_master` carries a `/-  auto_smb` direct-map line. The role writes that line with `lineinfile`. **macOS updates reset `/etc/auto_master` and strip it, so re-run `make run hosts=<host> tags=smb-mount` after every OS update.**
