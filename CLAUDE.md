...
- **Register autofs SMB map in `/etc/auto_master`** — sshd's sandbox blocks direct writes to `/etc/auto_master`. The `generic/smb-mount` role works around this by installing a small LaunchDaemon (`local.smb-mount-fixup`) that runs at boot in launchd's domain (not under sshd) and appends the line if missing. Same daemon self-heals after macOS OS updates, which reset `/etc/auto_master` to the stock system default. No manual step required on present-side. Teardown (`smb_mount_state=absent`) still prompts for a GUI-Terminal one-liner since the daemon itself is being removed.
