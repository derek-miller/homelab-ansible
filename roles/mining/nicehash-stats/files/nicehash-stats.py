...
def nicehash_request(
    method, path, api_key=None, api_secret=None, org_id=None, **kwargs
):
    ...
    try:
        ...
        for mining_rig in data.get("miningRigs", []):
            ...
            for rig_stat in mining_rig.get("stats", []):
                ...
                market_factor = float(
                    algorithm_stats.get(algorithm, {}).get("marketFactor", 0)
                )
                mining_factor = float(
                    algorithm_stats.get(algorithm, {}).get("miningFactor", 0)
                )
                bitcoin_per_day = (
                    speed_accepted
                    * mining_factor
                    ...
                    to_line_protocol(
                        ...
                        fields={
                            ...
                            "rejected_target": float(
                                rig_stat.get("speedRejectedR1Target", 0)
                            ),
                            "rejected_stale": float(
                                rig_stat.get("speedRejectedR2Stale", 0)
                            ),
                            "rejected_duplicate": float(
                                rig_stat.get("speedRejectedR3Duplicate", 0)
                            ),
                            "rejected_time": float(
                                rig_stat.get("speedRejectedR4NTime", 0)
                            ),
                            "rejected_other": float(
                                rig_stat.get("speedRejectedR5Other", 0)
                            ),
                            "rejected_total": float(
                                rig_stat.get("speedRejectedTotal", 0)
                            ),
                        },
...
class ClickNicehashAlgorithmChoice(click.Choice):
    ...
    def convert(self, value, param, ctx):
        if not self.choices:
            ...
            response.raise_for_status()
            self.choices = sorted(
                {a["algorithm"] for a in response.json()["miningAlgorithms"]}
            )
...
class ClickRegex(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        try:
            return re.compile(
                value.encode("utf-8") if self._as_bytes else value, flags=self._flags
            )
...
@click.option(
    ...
    help="Index only resource events whose key matches this regex (ex: 'workflow:dev:dx.test.nop')",
)
@click.option("-k", "--kwh-cost", default=0.2, type=click.FLOAT)
@click.option(
    "-o",
    "--output",
    type=click.Path(
        exists=False, dir_okay=False, file_okay=True, allow_dash=True, writable=True
    ),
    default="-",
)
@click.option(
...
@click.pass_context
def profitability(
    ctx, categories, algorithms, device_regex, kwh_cost, output, output_format
):
    bitcoin_value = get_nicehash_exchange_rates(
        ctx.obj["api_key"], ctx.obj["api_secret"], ctx.obj["org_id"]
    )[("BTC", "USD")]
    ...
    for device_short in (response.json() or {}).get("devicesShort") or []:
        if (
            not device_short.get("name")
            or (categories and device_short["category"] not in categories)
            or (device_regex and not device_regex.match(device_short["name"]))
        ):
            continue
        response = nicehash_request(
            "get",
            "/main/api/v2/public/profcalc/device",
            params={"device": device_short["id"]},
        )
        response.raise_for_status()
            ...
            device_profitability.append(
                {
                    ...
                    "kwh_cost": kwh_cost,
                    "power_cost_per_day": locale.currency(cost_per_day),
                    "bitcoin_per_day": bitcoin_per_day,
                    "revenue_per_day": locale.currency(revenue_per_day),
                    "profit_per_day": locale.currency(revenue_per_day - cost_per_day),
                    "bitcoin_value": locale.currency(bitcoin_value),
                    "profitability": round(
                        (revenue_per_day / cost_per_day) * 100, ndigits=2
                    ),
                }
            )
    # Sort by profitability
    device_profitability.sort(key=lambda p: p["profitability"], reverse=True)
